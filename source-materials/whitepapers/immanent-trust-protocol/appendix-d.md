...
| Max neighborhood size | \(d_{\max}\) | 100 | 4.2.1 | Limits storage and computation; sampling ensures statistical representativeness. |
...
| Parameter | Symbol | Default | Section | Rationale |
|-----------|--------|---------|---------|-----------|
| Velocity limit (low‑stakes trust per day) | \(\Delta_{\max}\) | 0.05 | 6.3.1 | Reaches τ=0.8 in 16 days; deters Sybil farming. |
| Max confidence for low‑stakes signals | \(c_{\max}^{\text{(low)}}\) | 0.3 | 6.3.2 | Low‑stakes interactions are inherently low‑information. |
| High‑stakes trust threshold | \(\tau_{\text{high}}\) | 0.8 | 6.3.1 | Requires substantial accumulated trust for sensitive actions. |
...
| Micro‑payment (optional) | \(c_{\text{low}}\) | $0.001 | 6.4.1 | Economic deterrent for large‑scale Sybil attacks. |
...
| Propagation decay factor | \(\gamma\) | 0.5 | 5.2.2 | Each hop halves the penalty. |
...
| Parameter | Symbol | Default | Section | Rationale |
|-----------|--------|---------|---------|-----------|
| Minimum members to form SAF | \(K_{\text{min}}\) | 5 | 7.2.4 | Avoids small, easily colluded groups. |
...
| Parameter | Symbol | Default | Section | Rationale |
|-----------|--------|---------|---------|-----------|
| Number of relays per node | \(N_{\text{relay}}\) | 8 | 9.5.2 | Eclipse probability < 10⁻⁸ with 10% adversarial relays. |
