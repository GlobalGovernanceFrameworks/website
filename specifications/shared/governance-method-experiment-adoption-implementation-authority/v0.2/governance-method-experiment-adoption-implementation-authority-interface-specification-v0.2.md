...
version: 0.2
specification_id: GMEAIA/0.2
status: recursive-systems-audit-draft-successor
date: 2026-08-05
constitutional_interface: Constitutional Interface Specification v0.2
---

# Governance Method, Experiment, Adoption, and Implementation Authority Interface Specification v0.2

## A shared lifecycle grammar for methods, experiments, authority, response, implementation, learning, and closure

**Specification ID:** `GMEAIA/0.2`  
**Status:** Draft successor to `GMEAIA/0.1` for adversarial review and stack migration  
...
**Constitutional dependency:** Constitutional Interface Specification v0.2  
**Compatibility:** Retains all sixty v0.1 record IDs, eighteen action classes, ten authority-effect labels, and the forty-eight-field common header
...
`GMEAIA/0.2` supplies a common type system for governance records that move from signal to proposal, consent, authority, activation, implementation, review, scaling, remedy, and closure.
...
**consequence profile.** The proportional record bundle selected according to reversibility, rights, coercion, service effects, affected outsiders, ecological irreversibility, data sensitivity, duration, and scale.

**constitutional gap.** A consequential condition for which no identified authority possesses sufficient lawful jurisdiction and practical capacity.
...
Responsibility should follow lawful jurisdiction, causal control, affected standing, practical capacity, and accompanying resources. The location where consequences appear does not alone determine responsibility.

## 3.4 Duty attached to retained jurisdiction

Where law, charter, or compact establishes a response duty, the competent authority shall acknowledge and accept, refuse, defer, or refer a consequential request within a decision-appropriate period. Delay does not create authority transfer, but it does not erase responsibility for avoidable non-action.
...
Consequential handoffs additionally distinguish:

`offered`, `acknowledged`, `accepted`, `partially_accepted`, `declined`, `deferred`, `timed_out`, `misrouted`, `constitutional_gap`, `returned`, and `closed`.
...
### P0 — Exploratory and non-consequential

Examples: simulation, public education, optional brainstorming, non-operative draft.

Minimum bundle: `GMEAIA-02`, `03`, `58`, and data or closure records where applicable. Records may be combined into one accessible form.

### P1 — Voluntary bounded practice
...
Typical bundle: `02`, `03`, `09`, `11`, `14`, `18`–`19`, `24`, `28`, `34`, `38`–`41`, and `60` as applicable.

### P2 — Operational or consequential action

May affect workload, services, public resources, institutional operations, affected outsiders, or material rights.

Typical bundle: relevant records from `01`, `05`–`13`, `18`–`42`, `50`–`54`, and `59`–`60`. The exact bundle is justified through `GMEAIA-20`.

### P3 — Protected-domain or constitutional action

Affects coercion, essential services, status, title, irreversible ecology, children, emergency authority, adjudication, taxation, benefit withdrawal, or high-risk technology.

Requires P2 plus `GMEAIA-56` or `57` and the controlling specialized interface. Ordinary GMEAIA conformance is never sufficient by itself.

## 6.7 Framework falsification profile

Framework-level review is carried primarily by `GMEAIA-53`, supported by `30`, `32`, `43`, and `45`. It shall state:
...
A validated matter may be referred to a candidate authority. The referral records acknowledgment and decision clocks, acceptance or refusal, reasons, conflicts, expected harm during delay, sender and recipient duties, escalation, and constitutional-gap status.
...
- acceptance, partial acceptance, refusal, deferment, onward referral, or timeout;
- reasons, conflicts, missing prerequisites, and expected harm during delay;
- sender duty, recipient duty, custody, escalation, and constitutional-gap state;
...
- affected parties, burdens assigned, and resources transferred;
...
**Lifecycle relation:** Fail-closed gate before trial authorization and required for consequential transfer, devolution, reform mandate, or system-effect pilot.
...
- acknowledgment and decision deadlines;
- acceptance, partial acceptance, refusal, deferment, timeout, or constitutional-gap state;
...
A deployment claiming `GMEAIA/0.2` conformance shall satisfy all applicable gates.
...
| G03 | Consequence profile P0–P3 is selected and justified. |
...
| G20 | Every operational framework states disconfirming evidence and narrowing, suspension, replacement, or non-scaling conditions. |
...
# 10. Compound tests added in v0.2
...
| T05 | A voluntary workshop is required to complete a full P3 record bundle | Non-conformant over-administration; P0 or P1 applies. |
| T06 | A technically simple pilot affects household water access | P3 protected-domain route applies despite technical simplicity. |
...
**Acknowledgment due:**  
**Decision due:**  
**State:** offered / acknowledged / accepted / partially accepted / declined / deferred / timed out / misrouted / constitutional gap  
...
`GMEAIA/0.2` preserves the central achievement of v0.1: methods, pilots, funding, technology, and coordination cannot counterfeit authority.
