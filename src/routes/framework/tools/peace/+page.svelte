<!-- src/routes/framework/tools/peace/+page.svelte -->
<script>
  ...
  const getFilePath = (toolName) => {
    ...
    if (toolName.includes('-sv') || toolName.includes('-en')) {
      return `${base}/framework/tools/peace/${toolName}`;
    }
    ...
    if (toolName.includes('guide') || 
      ...
      if (toolName === 'youth-peace-action-guide.pdf' || toolName === 'youth-peace-action-guide.md') {
        ...
        return `${base}/framework/tools/peace/youth-peace-action-guide${langSuffix}.${toolName.split('.').pop()}`;
      } else {
        // For other translated documents
        const extension = toolName.split('.').pop();
        const baseName = toolName.substring(0, toolName.lastIndexOf('.'));
        return `${base}/framework/tools/peace/${baseName}-${currentLocale}.${extension}`;
      }
    }
          ...
          <div class="tool-card {tool.color}">
            ...
            <div class="tool-actions">
              {#if tool.isPageLink}
                ...
                <a href="{base}/framework/tools/peace/systems-thinking" class="page-link-btn">
