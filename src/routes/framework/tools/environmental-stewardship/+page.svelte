<!-- src/routes/framework/tools/environmental-stewardship/+page.svelte -->
<script>
  ...
  const getFilePath = (toolName, category = 'environmental-stewardship') => {
    ...
    if (toolName.includes('one-page-essence') || toolName.includes('first-100-days')) {
      ...
      return `${base}/framework/tools/environmental-stewardship/${baseName}-${currentLocale}.${extension}`;
    }
