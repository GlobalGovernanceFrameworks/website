<!-- src/routes/framework/tools/economic/+page.svelte -->
<script>
  ...
  const getFilePath = (toolName) => {
    ...
    if (toolName.includes('-sv') || toolName.includes('-en')) {
      return `${base}/framework/tools/economic/${toolName}`;
    }
    ...
    if (toolName === 'digital-ethics.pdf' || toolName === 'digital-ethics.md') {
      ...
      return `${base}/framework/tools/economic/digital-ethics${langSuffix}.${toolName.split('.').pop()}`;
    }
    ...
    if (toolName.includes('guide') || 
      ...
      if (toolName === 'youth-economic-action-guide.pdf' || toolName === 'youth-economic-action-guide.md') {
        const langSuffix = currentLocale === 'sv' ? '-sv' : '-en';
        return `${base}/framework/tools/economic/youth-economic-action-guide${langSuffix}.${toolName.split('.').pop()}`;
      } else {
        // For other translated documents
        const extension = toolName.split('.').pop();
        const baseName = toolName.substring(0, toolName.lastIndexOf('.'));
        return `${base}/framework/tools/economic/${baseName}-${currentLocale}.${extension}`;
      }
    }
          ...
          <div class="tool-card {tool.color}">
            ...
            <div class="tool-actions">
              {#if tool.isPageLink}
                ...
                <a href="{base}/framework/tools/economic/{tool.id}" class="page-link-btn">
