<!-- src/routes/framework/tools/governance/+page.svelte -->
<script>
  ...
  const getFilePath = (toolName) => {
    ...
    if (toolName.includes('-sv') || toolName.includes('-en')) {
      return `${base}/framework/tools/governance/${toolName}`;
    }
    ...
    if (['decidetogether-platform-guide',
      ...
      return `${base}/framework/tools/governance/${baseName}-${currentLocale}.${extension}`;
    }
