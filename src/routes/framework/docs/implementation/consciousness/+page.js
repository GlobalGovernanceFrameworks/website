...
export async function load({ depends, url }) {
  ...
  try {
    ...
    for (const section of sections) {
      try {
        ...
        content[section] = await import(`$lib/content/framework/${currentLocale}/implementation/consciousness/${section}.md`);
        isModular = true;
      } catch (e) {
        // Fall back to English if translation isn't available
        try {
          content[section] = await import(`$lib/content/framework/en/implementation/consciousness/${section}.md`);
          isModular = true;
        } catch (e2) {
  ...
  if (!isModular) {
    try {
      legacyContent = await import(`$lib/content/framework/${currentLocale}/implementation/consciousness.md`);
    } catch (e) {
      try {
        legacyContent = await import(`$lib/content/framework/en/implementation/consciousness.md`);
      } catch (e2) {
