...
export async function load({ depends, url }) {
  ...
  try {
    ...
    for (const section of sections) {
      try {
        ...
        content[section] = await import(`$lib/content/framework/${currentLocale}/implementation/peace/${section}.md`);
        isModular = true;
      } catch (e) {
        // Fall back to English if translation isn't available
        try {
          content[section] = await import(`$lib/content/framework/en/implementation/peace/${section}.md`);
          isModular = true;
        } catch (e2) {
  ...
  if (!isModular) {
    try {
      legacyContent = await import(`$lib/content/framework/${currentLocale}/implementation/peace.md`);
    } catch (e) {
      try {
        legacyContent = await import(`$lib/content/framework/en/implementation/peace.md`);
      } catch (e2) {
