...
<script>
  ...
  import enFallback from '$lib/i18n/en/outreach.json';
  ...
  async function loadMarkdownContent(loc) {
    ...
    try {
      const module = await import(`$lib/content/get-involved/outreach/${loc}/outreach-guide.md`);
      ...
      try {
        ...
        console.error('Failed to load outreach guide:', e2);
        loadError = true;
      }
    }
  ...
  function getText(key) {
    const value = $t(`outreach.${key}`);
    if (value) return value;
    return fromFallback(key) || '';
  }

  // Outreach contributors need the material, not the repository.
</script>
    ...
    <div class="main-content">
      ...
      {:else if loadError || !guideContent}
        <div class="error-state">
          ...
          <a href="{base}/frameworks" class="primary-btn">
            {getText('exploreFrameworks')}
          </a>
        </div>
      {:else}
    ...
    <div class="bottom-cta">
      <div class="cta-content">
        ...
        <div class="cta-actions">
          <a href="{base}/frameworks" class="primary-btn large">
            {getText('exploreFrameworks')}
          </a>
          <a href="{base}/blog" class="primary-btn large secondary">
            {getText('readBlog')}
          </a>
        </div>
      </div>
