// src/routes/frameworks/docs/implementation/environmental-stewardship/+page.js
...
export async function load({ depends, url }) {
  ...
  try {
    ...
    for (const tool of tools) {
      try {
        // Try to load the current locale version
        toolsContent[tool] = await import(`$lib/content/frameworks/${currentLocale}/tools/environmental-stewardship/${tool}.md`);
      } catch (e) {
        // Fall back to English if translation isn't available
        try {
          toolsContent[tool] = await import(`$lib/content/frameworks/en/tools/environmental-stewardship/${tool}.md`);
        } catch (e2) {
  ...
  if (!isModular) {
    try {
      legacyContent = await import(`$lib/content/frameworks/${currentLocale}/implementation/environmental-stewardship.md`);
    } catch (e) {
      try {
        legacyContent = await import(`$lib/content/frameworks/en/implementation/environmental-stewardship.md`);
      } catch (e2) {
