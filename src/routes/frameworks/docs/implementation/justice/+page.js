// src/routes/frameworks/docs/implementation/justice/+page.js
...
export async function load({ depends, url }) {
  ...
  try {
    ...
    for (const section of sections) {
      try {
        ...
        content[section] = await import(`$lib/content/frameworks/${currentLocale}/implementation/justice/${section}.md`);
        isModular = true;
      } catch (e) {
        // Fall back to English if translation isn't available
        try {
          content[section] = await import(`$lib/content/frameworks/en/implementation/justice/${section}.md`);
          isModular = true;
  ...
  if (!isModular) {
    try {
      legacyContent = await import(`$lib/content/frameworks/${currentLocale}/implementation/justice.md`);
    } catch (e) {
      try {
        legacyContent = await import(`$lib/content/frameworks/en/implementation/justice.md`);
      } catch (e2) {
