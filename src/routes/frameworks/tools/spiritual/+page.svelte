<!-- src/routes/frameworks/tools/spiritual/+page.svelte -->
<script>
  ...
  const getFilePath = (toolName) => {
    ...
    if (toolName.includes('-sv') || toolName.includes('-en')) {
      return `${base}/frameworks/tools/spiritual/${toolName}`;
    }
    ...
    if (toolName.includes('executive-brief') || 
      ...
      return `${base}/frameworks/tools/spiritual/${baseName}-${currentLocale}.${extension}`;
    }
        ...
        {#each coreTools as tool}
          <div class="tool-card {tool.color}">
            {#if tool.id === 'multimedia-resource-library'}
              ...
              <div class="tool-actions">
                <a href="{base}/frameworks/tools/spiritual/multimedia-resource-library" class="view-btn">
