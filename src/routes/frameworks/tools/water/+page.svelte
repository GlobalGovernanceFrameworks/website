<!-- src/routes/frameworks/tools/water/+page.svelte -->
<script>
  ...
  const getFilePath = (toolName) => {
    ...
    if (toolName.includes('-sv') || toolName.includes('-en')) {
      return `${base}/frameworks/tools/water/${toolName}`;
    }
    ...
    if (toolName.includes('sacred-waters-kit') || toolName === 'sacred-waters-kit.zip') {
      ...
      if (toolName === 'sacred-waters-kit.zip') {
        ...
        return `${base}/frameworks/tools/water/sacred-waters-kit${langSuffix}.zip`;
      }
    }
    ...
    if (toolsWithTranslations.includes(toolName.split('.')[0])) {
      ...
      return `${base}/frameworks/tools/water/${baseName}-${currentLocale}.${extension}`;
    }
