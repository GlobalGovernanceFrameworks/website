<!-- src/routes/frameworks/tools/digital/+page.svelte -->
<script>
  ...
  const getFilePath = (toolName) => {
    ...
    if (toolName.includes('-sv') || toolName.includes('-en')) {
      return `${base}/frameworks/tools/digital/${toolName}`;
    }
    ...
    if (toolName === 'digital-ethics-guide.pdf' || toolName === 'digital-ethics-guide.md') {
      ...
      return `${base}/frameworks/tools/digital/digital-ethics-guide${langSuffix}.${toolName.split('.').pop()}`;
    }
    ...
    if (toolName.includes('guide') || 
      ...
      if (toolName === 'youth-guide.pdf' || toolName === 'youth-guide.md') {
        const langSuffix = currentLocale === 'sv' ? '-sv' : '-en';
        return `${base}/frameworks/tools/digital/youth-guide${langSuffix}.${toolName.split('.').pop()}`;
      } else {
        // For other translated documents
        const extension = toolName.split('.').pop();
        const baseName = toolName.substring(0, toolName.lastIndexOf('.'));
        return `${base}/frameworks/tools/digital/${baseName}-${currentLocale}.${extension}`;
      }
    }
          ...
          <div class="tool-card {tool.color}">
            ...
            <div class="tool-actions">
              {#if tool.isPageLink}
                ...
                <a href="{base}/frameworks/tools/digital/{tool.id}" class="page-link-btn">
                  {texts.viewTool}
                  <span class="icon">→</span>
                </a>
